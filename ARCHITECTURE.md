...
| `workouts.status` | text, default `'completed'`, check `in ('scheduled','completed')` (added 2026-07-27) — the sync window now also covers today through Saturday of the *current* week (still `LOOKBACK_DAYS` back for the past), and stamps any event whose start time hasn't happened yet as `'scheduled'` so the dashboard can show it before it's done. There's no separate "mark as attended" step — same ground truth as always (a calendar event = a workout), just extended one week forward. Once an event's date is in the past the next day's sync (or the dashboard's own client-side check) treats it as `'completed'` regardless of what was last written, so a `'scheduled'` status never gets stuck. | |
